  ...
  private async generateWithConfig(prompt: string, modelName: AvailableModel, useSearch: boolean = false, isInitialGeneration: boolean = true): Promise<any> {
    try {
      ...
      if (modelName === 'gemini-2.5-flash-preview-04-17') {
         // Example: Disable thinking for low latency if needed for this specific model variant.
         // modelRequestConfig.thinkingConfig = { thinkingBudget: 0 }; 
      ...
      try {
        ...
        // Allow for array of objects as well as single object
      ...
      if (isInitialGeneration) {
        ...
        if (!faultInfo.type) { 
            ...
            // chosen_transition validation removed from here as it's per-slide
      ...
      if (error instanceof Error) {
        if (error.message.startsWith("AI_SERVICE_ERROR:") || 
            ...
            error.message.includes("got status: 400 INVALID_ARGUMENT") || // Specific error for bad requests
            error.message.includes("GoogleGenerativeAI Error")) { // General SDK errors
            throw error;
        }
...
- "image_suggestions": Array of objects for image placements: { "slide_reference": "e.g., Slide 2, Concept X", "description": "intended image" }.
...
Example JSON for "html_content" demonstrating these rules:
{
  ...
  "chosen_theme": "sky",
  "image_suggestions": [
    { "slide_reference": "Example: Slide 1, Title Visual", "description": "A dynamic visual..." }
  ],
...
1. Input:
   ...
   - User Image Inputs:
     ...
     - If an image \`url\` is a standard HTTP/HTTPS link, use it directly.
     - If an image \`url\` is a placeholder like \`ai_image_ref:INDEX\` (e.g., \`ai_image_ref:0\`), this signifies an AI-generated image. You MUST use this exact placeholder string (e.g., \`ai_image_ref:0\`) as the value in the \`src\` attribute of an \`<img>\` tag, or within a \`url()\` for a CSS background-image property (e.g., \`style='background-image: url(ai_image_ref:0)'\` or \`data-background-image='ai_image_ref:0'\`).
     - 'ai_suggested': Insert image (using its \`url\` or placeholder) at conceptual place related to 'suggestion_reference' within a slide.
     - 'user_defined_existing_slide': Add image (using its \`url\` or placeholder) to slide \`slide_number\`.
     - 'user_defined_new_slide': Create a new \`<section>\` with the image (using its \`url\` or placeholder). This new slide MUST be inserted after the slide whose number is \`after_slide_number\`.
...
2. HTML Structure for "enhanced_html_content":
   ...
   - PER-SLIDE TRANSITIONS: Review and update the 'data-transition' attribute on each <section> tag as needed based on content and user requests. You can use transitions like: 'none', 'fade', 'slide', 'convex', 'concave', 'zoom', 'page', 'cube', 'coverflow', 'concave-cube', 'convex-cube', 'fade-in-then-out', 'fade-out-then-in'. You can also conceptually choose 'random' for a slide, in which case you (the AI) must pick a suitable varied transition from this list for that specific slide and use that concrete value in the 'data-transition' attribute. The goal is varied and engaging per-slide transitions. Preserve existing 'data-transition' attributes if they are still appropriate. Example: <section data-transition='page'>...</section>.

3. Visual Polish & Engagement:
   - Refine typography, layout, color scheme, and use advanced Reveal.js features like fragments.
   - Content Legibility is highest priority. Ensure readability even with scrolling.

4. Image Integration:
   - Integrate images using \`<img>\` tags or \`data-background-image\` attributes on \`<section>\`s. Use placeholders like \`ai_image_ref:index\` for AI-generated images. Provide meaningful 'alt' attributes.
...
{
  "enhanced_html_content": "<div class='reveal'><div class='slides'><section data-transition='cube' data-background-image='ai_image_ref:0' class='custom-slide-class' style='background-color: lightblue;'><h1 style='font-family: Arial;'>Updated Slide 1</h1><p>New content with image <img src='https://example.com/image.png' alt='description' style='width:50%;'> and text with a \\"quote\\" and an AI image <img src='ai_image_ref:1' alt='description from user input'></p></section></div></div>",
  "ai_confirmation_or_further_queries": "Images integrated (using placeholders for AI-generated ones), per-slide transitions updated, visual style refreshed. Anything else?"
...
2.  For each slide:
    ...
    e.  If an image (\`<img>\`) is present, include its \`alt\` text as "Image: [alt text]". If its \`src\` attribute is an AI placeholder like \`ai_image_ref:index\`, represent this as "Image: [AI-Generated Image Placeholder (was: ai_image_ref:index) - Alt: alt text]".
    f.  If a section has a \`data-background-image\`, mention it. If the value is an AI placeholder like \`ai_image_ref:index\`, state "Background Image: [AI-Generated Image Placeholder (was: ai_image_ref:index)]". Otherwise, state "Background Image: [URL]".
...
Image: A diagram illustrating the relationship between AI, ML, and DL.
  ...
  public async generateImageFromPrompt(prompt: string): Promise<string> {
    try {
      ...
      if (error instanceof Error) {
        ...
        // Check for specific error types from the SDK if available, otherwise use a generic message
